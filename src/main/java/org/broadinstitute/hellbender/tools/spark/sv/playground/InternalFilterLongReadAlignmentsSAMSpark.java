...
public class InternalFilterLongReadAlignmentsSAMSpark extends GATKSparkTool {
    ...
    protected void runTool(final JavaSparkContext ctx) {
        ...
        FileUtils.writeLinesToSingleFile(
                filterByScore(reads, header, localLogger)
    ...
    /**
     * Filters an input of SAM file containing alignments of single-ended long read that
    ...
    static JavaRDD<AlignedContig> filterByScore(final JavaRDD<GATKRead> longReads,
        ...
        return filterAndSplitGappedAI(parsedContigAlignments);
    }
     ...
     *  *) when 1 contig is yield, it means the contig has only 1 configuration that scored the best
     *  *) when multiple contigs are yield, it means the contig has several top-scored configurations
    ...
    static JavaRDD<AlignedContig> filterAndSplitGappedAI(final JavaRDD<AlignedContig> parsedContigAlignments) {
        ...
        final Set<String> canonicalChromosomes = getCanonicalChromosomes();
        ...
        return parsedContigAlignments
                ...
                .sortBy(tig -> tig.contigName, true, parsedContigAlignments.getNumPartitions());
    }
    ...
    static List<List<AlignmentInterval>> pickBestConfigurations(final AlignedContig alignedContig,
        ...
        final List<List<AlignmentInterval>> result = new ArrayList<>();
        for(int i = 0; i < allConfigurations.size(); ++i) {
            if (scores.get(i) >= maxScore)
                result.add(allConfigurations.get(i));
        }
    ...
    private static Iterator<AlignedContig> reConstructContigFromPickedConfiguration(
            final Tuple2<String, Tuple2<byte[], List<List<AlignmentInterval>>>> forOneContig) {
        if (forOneContig._2._2.size() > 1) {
     ...
     */
    static Set<String> getCanonicalChromosomes() {
